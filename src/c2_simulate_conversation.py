...
#     "llama-3.2-3B-lora": PROJECT_PATH.joinpath("model/gguf/llama3.2-3B-lora"),
#     "llama-3.2-3B-lora-ppo": "/home/htran/generation/med_preferences/AgentClinic/ppo_checkpoints/checkpoint-60",
...
def query_model(model_str, prompt, system_prompt, conversation_history=[], tries=30, timeout=20.0, image_requested=False, scene=None,
    ...
    if model_str not in ["gpt4", "gpt3.5", "gpt4o", 'llama-2-70b-chat', 'llama-3-3b-instruct', 'llama-3-1b-instruct', "mixtral-8x7b", "gpt-4o-mini",
                         "llama-3-70b-instruct", "gpt4v", "claude3.5sonnet", "o1-preview", 'llama3.2-3B', 'llama3.2-3B-lora', 'llama3.2-3B-sft','llama3.2-3B-lora-ppo', 'llama3.2-3B-lora-grpo', 'HF_llama3.2-3B', 'HF_llama3.2-3B-lora', 'HF_llama3.2-3B-sft','HF_llama3.2-3B-lora-ppo', 'HF_llama3.2-3B-lora-grpo',"HF_meta-llama/Meta-Llama-3.1-8B-Instruct"] and "HF_" not in model_str:
        ...
        try:
            ...
            if model_str == "gpt4":
                ...
                response = client.chat.completions.create(
                    ...
                    temperature=0.05,
                    max_tokens=200,
                    stop=["END OF CONVERSATION", "<end of conversation>"],
                )
                answer = response.choices[0].message.content
                answer = re.sub("\s+", " ", answer)
            ...
            # llama-3-3B
            elif model_str == 'llama3.2-3B':
                client = OpenAI(base_url="http://0.0.0.0:2999/v1", api_key="original")
                messages = [
                    {"role": "system", "content": system_prompt}
                ]
                messages.extend(conversation_history)
                ...
                response = client.chat.completions.create(
                    ...
                    # extra_body={
                        ...
                        # "frequency_penalty" : 2,
                        # "length_penalty" : -3,
                        # "stop" : ["END OF CONVERSATION", "<end of conversation>", "end of conversation"]
                        # },
                )
                answer = response.choices[0].message.content
                answer = re.sub("\s+", " ", answer)
            ...
            # llama-3-3B-lora 
            elif model_str == 'llama3.2-3B-lora':
                client = OpenAI(base_url="http://0.0.0.0:3999/v1", api_key="original")
                messages = [
                    {"role": "system", "content": system_prompt}
                ]
                messages.extend(conversation_history)
            ...
            # llama-3-3B-sft
            elif model_str == 'llama3.2-3B-sft':
                client = OpenAI(base_url="http://0.0.0.0:3999/v1", api_key="original")
                messages = [
                    {"role": "system", "content": system_prompt}
                ]
                ...
                answer = re.sub("\s+", " ", answer)
            # lora-RL
            elif model_str == 'llama3.2-3B-lora-ppo':
            ...
            elif model_str == 'llama3.2-3B-lora-grpo':
                client = OpenAI(base_url="http://0.0.0.0:5999/v1", api_key="original")
                messages = [
                    {"role": "system", "content": system_prompt}
                ]
                messages.extend(conversation_history)
...
class GoldEHRNote:
    def __init__(self) -> None:
        self.scenarios = []
        with open("/home/wjang/2024_chatbot_noteaid/data/processed/gold.json", "r") as f:
            data = json.load(f)
...
class SilverEHRNote:
    def __init__(self) -> None:
        self.scenarios = []
        with open("/home/wjang/2024_chatbot_noteaid/data/processed/synthetic_notes_for_test.json", "r") as f:
            data = json.load(f)
...
class PatientAgent:
    def __init__(self, scenario, backend_str="gpt-4o-mini", bias_present=None, literacy_level=None) -> None:
    ...
    def inference_patient(self, question) -> str:
        ...
        #     )
        #
        # answer = query_model(self.backend, "", self.system_prompt(), conversation_history=self.conversation_history, scene=self.scenario, pipe=self.pipe)
        #
        # if ("||END OF CONVERSATION" in answer) or ("<end of conversation>" in answer) :
        #     return answer
    ...
    def system_prompt(self) -> str:
        ...
        base = """You are a patient who has recently been discharged from a clinic visit. You are having a conversation with your doctor to better understand the information in your discharge note. Your responses should be in the form of dialogue, consisting of 1-2 sentences."""
...
class DoctorAgent:
    ...
    def system_prompt(self) -> str:
        ...
        if self.bias_present is not None:
            bias_prompt = self.generate_bias()
        # base = "You are a medical expert who is having a conversation with a patient to help them understand the key details of their discharge note. You will explain their diagnosis, treatment plan, medications, procedures during the hospital stay and follow-up instructions in a clear and supportive manner. You are only allowed to ask {} questions total to ensure the patient understands their care. You have asked {} questions so far. Your dialogue will be 1-2 sentences in length, and you should encourage the patient to ask questions if anything is unclear."#.format(self.MAX_INFS, self.infs)

        # base = config.load_prompts("autoeval_stage1.txt")
        base = """
        You are a medical expert who is having a conversation with a patient to help them understand the key details of their discharge note. You will explain their diagnosis, treatment plan, medications, procedures during the hospital stay and follow-up plans and any sign/symptom when the patient should come back or call the hospital. Please do this in a clear and supportive manner. Your dialogue will be 1-2 sentences in length, and you should encourage the patient to ask questions if anything is unclear. 
        """#.format(self.MAX_INFS, self.infs)
        presentation = "\n\nBelow is the discharge note of the patient: {}. \n\n".format(
            self.discharge_note)
        return base + bias_prompt + presentation
        # return base.format(discharge_note=self.discharge_note)
...
def main(api_key, replicate_api_key, inf_type, doctor_bias, patient_bias, doctor_llm, patient_llm, examiner_llm,
    ...
    if dataset == "MedQA":
        ...
        scenario_loader = GoldEHRNote()
    elif dataset == "silver" :
        scenario_loader = SilverEHRNote()
    else:
    ...
    for _scenario_id in range(0, min(num_scenarios, scenario_loader.num_scenarios)):
        ...
        conv_history[_scenario_id] = scenario
        ...
        # append for recording
        conv_history['conversation'] = doctor_agent.conversation_history
        CONVERSATION_HISTORY.append(conv_history)

    with open(DATA_PATH.joinpath(f"evaluation_conversation_history_{doctor_llm}_{dataset}.pkl"), 'wb') as f :
...
if __name__ == "__main__":
    ...
    parser.add_argument('--total_inferences', type=int, default=20, required=False,
